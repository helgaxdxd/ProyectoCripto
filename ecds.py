...
class SignatureApp(tk.Tk):
    ...
    def create_widgets(self):
        ...
        self.dh_secret_file_entry = tk.Entry(decrypt_frame, width=50, show="*")
        self.dh_secret_file_entry.grid(row=1, column=0, padx=5, pady=5)

        self.dh_secret_file_button = tk.Button(decrypt_frame, text="Browse DH Secret File", command=self.browse_dh_secret_file)
        self.dh_secret_file_button.grid(row=1, column=1, padx=5, pady=5)
    ...
    def browse_dh_secret_file(self):
        dh_secret_file_path = filedialog.askopenfilename(filetypes=[("Text files", "*.txt")])
        self.dh_secret_file_entry.delete(0, tk.END)
        self.dh_secret_file_entry.insert(0, dh_secret_file_path)
...
def decrypt_file_dh(self):
    ...
    dh_secret_file_path = self.dh_secret_file_entry.get()
    try:
        with open(file_path, "rb") as f:
            iv = base64.b64decode(f.readline().strip())
            ciphertext = base64.b64decode(f.read())

        with open(dh_secret_file_path, "rb") as f:
            dh_secret = f.read()

        # Generate AES key from DH secret
        aes_key = sha256(dh_secret).digest()[:16]  # Use the first 16 bytes of DH secret as AES key
